...
const TOOLS = [
  ...
  { name: 'read_direct_messages', action: 'read_dm', description: 'Read messages in a DM this agent is part of (to find a message id, e.g. to delete one). Identify the thread by conversation_id, or by the counterpart (with_user_id / with_email).', schema: { type: 'object', properties: { conversation_id: { type: 'string' }, with_user_id: { type: 'string' }, with_email: { type: 'string' }, limit: { type: 'integer' }, include_deleted: { type: 'boolean' } } } },
  ...
  { name: 'delete_message', action: 'delete_message', description: 'Delete one or more messages (channel messages or DMs the agent participates in). Pass message_id or message_ids (up to 50). Set only_own:true to delete only AI-posted messages.', schema: { type: 'object', properties: { message_id: { type: 'string' }, message_ids: { type: 'array', items: { type: 'string' } }, only_own: { type: 'boolean' } } } },
  ...
  { name: 'delete_channel', action: 'delete_channel', description: 'Archive (soft-delete) a channel by slug, name, or id. Announcement/urgent channels are protected.', schema: { type: 'object', required: ['channel'], properties: { channel: { type: 'string' } } } },
  { name: 'register_webhook', action: 'register_webhook', description: 'Register/refresh an outbound webhook + command channel for a project. The channel becomes a two-way console: messages there are read as instructions and replies post back.', schema: { type: 'object', required: ['url'], properties: { url: { type: 'string' }, project_name: { type: 'string' }, secret: { type: 'string' }, events: { type: 'array', items: { type: 'string' } } } } },
...
async function handleRpc(msg, token) {
  ...
  if (method === 'initialize') return reply({ protocolVersion: msg?.params?.protocolVersion ?? PROTOCOL_VERSION, capabilities: { tools: { listChanged: false } }, serverInfo: { name: 'rop-chat', version: '1.1.0' }, instructions: 'ROP Chat tools. Read, post, and manage messages, DMs, channels, tasks, and webhooks under this agent’s permissions (create/delete channels, delete messages, DMs).' })
